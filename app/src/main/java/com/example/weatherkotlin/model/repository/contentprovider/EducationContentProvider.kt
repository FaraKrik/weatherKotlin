package com.example.weatherkotlin.model.repository.contentprovider
...
import com.example.weatherkotlin.model.database.HistoryDao
...
class EducationContentProvider : ContentProvider() {
    ...
    private val URI_ALL = 1 // URI для всех записей
    private val URI_ID = 2 // URI для конкретной записи
    private val ENTITY_PATH = "HistoryEntity" // Часть пути (будем определять путь до HistoryEntity)
    ...
    private fun map(values: ContentValues?): HistoryEntity {
        return if (values == null) {
            ...
            val id = if (values.containsKey(HistoryEntity.ID)) values[HistoryEntity.ID] as Long else 0
